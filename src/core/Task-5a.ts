...
if(store !=null){
    var details:any = JSON.parse(store)
}
var clickTile:any
...
function disableButton(){
    var filledFormDetails:boolean = true;
    var nameField:any = $("#AddName").val();
    var mailField:any = $("#AddMail").val();
    var mobileField:any = $("#AddMobile").val();
    var landlineField:any = $("#AddLandline").val();
    var websiteField:any = $("#AddWebsite").val();
    var addressField:any = $("#AddAddress").val();
...
function displayContactContainerAdd(formDetails:any){
    var arr:any= [];
        var formDetails:any = {
            inputName: $("#AddName").val(),
            inputMail: $("#AddMail").val(),
        ...
        var display = `<div class="Display" id=${arr[0].inputName.replace(/\s/g, "")}>
                ...
                displayContainer.on('click', function(e){  
                    console.log(e);
                    let ids:any = e.target.parentNode;
                    let id = ids.id;
                    console.log(id);
                    //console.log(clickTile);
                    clickTile = arr.find((c: { inputName: string; }) => c.inputName.replace(/\s/g, "") === id);
                    console.log(clickTile);  
...
function displayContactContainer(details:any){
    let i;
        for(i=0; i<details.length; i++){
            ...
            displayContainer.on('click', function(e){     
                ...
                console.log(id);
                console.log(clickTile);
                clickTile = details.find((c: { inputName: string; }) => c.inputName.replace(/\s/g, "") === id);
                console.log(clickTile);
                $(".Display.active").removeClass("active");
                $("#"+clickTile.inputName.replace(/\s/g, "")).addClass("active");
                displayContactTile(clickTile); //errors
...
function displayContactTile(clickTile: any){
    $("#DetailsForm").css("visibility", "hidden");
